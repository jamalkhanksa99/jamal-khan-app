...
enum CategoryFilter { HOME, CENTER }

class CategoryController extends GetxController {
  final category = new Category().obs;
  final selected = Rx<CategoryFilter>(CategoryFilter.HOME);
  ...
  void toggleSelected(CategoryFilter filter) {
    ...
    if (isSelected(filter)) {
      selected.value = CategoryFilter.HOME;
    } else {
  ...
  Future loadEServicesOfCategory(String categoryId, {CategoryFilter filter}) async {
    try {
      ...
      switch (filter) {
        case CategoryFilter.HOME:
          _eServices = await _eServiceRepository.getAllWithPaginationFiltered(categoryId, "in_home");
          break;
        ...
        //   break;
        case CategoryFilter.CENTER:
          _eServices = await _eServiceRepository.getAllWithPaginationFiltered(categoryId, "in_center");
          break;
        default:
